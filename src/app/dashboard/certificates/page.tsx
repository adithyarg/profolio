...
import { Trash2, Award, FileText, CheckCircle2, AlertCircle, Pencil } from "lucide-react"
...
async function createCertificate(formData: FormData) {
    ...
    if (isUpdate) {
        ...
        const result = await supabase.from("certificates").insert([certificate])
        error = result.error
    }
    ...
    redirect(isUpdate ? "/dashboard/certificates?updated=1" : "/dashboard/certificates?success=1")
}
...
export default async function CertificatesPage({
    ...
    const editingCertificate = searchParams.edit 
        ? certificates?.find(c => c.id === searchParams.edit)
        : null

    // Parse date for editing
    let issueMonth = "", issueYear = ""
    if (editingCertificate?.issue_date) {
        const [month, year] = editingCertificate.issue_date.split(" ")
        issueMonth = month || ""
        issueYear = year || ""
    }
                        ...
                        {certificates.map((cert: any) => (
                            <div key={cert.id} className="group relative rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md hover:border-indigo-200 transition-all flex flex-col justify-between h-full">
                                <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <a 
                                        href={`/dashboard/certificates?edit=${cert.id}#form`}
            ...
            <div id="form" className="rounded-2xl border border-slate-200 bg-white shadow-sm p-6 sm:p-10">
                <div className="space-y-1 mb-8">
                    <h2 className="text-xl font-bold text-slate-900">
                        {editingCertificate ? 'Edit Certificate' : 'Add a Certificate'}
                    </h2>
                    <p className="text-sm font-medium text-slate-500">Add the details and optionally upload the PDF or image file.</p>
                    {editingCertificate && (
                        <a href="/dashboard/certificates" className="inline-block text-sm font-semibold text-indigo-600 hover:text-indigo-700 mt-2">
                            ← Cancel editing
                        </a>
                    )}
                </div>
                ...
                <form 
                    key={editingCertificate ? `edit-${editingCertificate.id}` : (searchParams?.success ? Date.now() : 'certificate-form')}
                ...
                >
                    {editingCertificate && (
                        <>
                            ...
                            <input type="hidden" name="existing_file_url" value={editingCertificate.file_url || ""} />
                        </>
                    )}
                    <div className="space-y-8">
                        ...
                        <div className="space-y-2 border-t pt-8">
                            <Label htmlFor="file" className="text-xs font-semibold uppercase tracking-wider text-slate-500">
                                Upload PDF or Image {editingCertificate ? '(Optional - leave empty to keep current file)' : '(Optional)'}
                            </Label>
                            {editingCertificate?.file_url && (
                                <p className="text-xs font-medium text-slate-600 mb-2">
                                    Current file: <a href={editingCertificate.file_url} target="_blank" rel="noreferrer" className="text-indigo-600 hover:underline">View</a>
                                </p>
                            )}
